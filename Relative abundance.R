...
library(ggpubr)
abundance <- read.csv("C:/Users/Robin.DESKTOP-0U5O684/Desktop/data/abundance.csv", header=T)
compaired <- list(c("Dry season", "Wet season"))
windowsFonts(TNM = windowsFont("Times New Roman"))
ggplot(abundance,aes(x=Season,y=Relative,fill=Season))+ #”fill=“设置填充颜色
...
stat_compare_means(comparisons=compaired, size=8,width=3,colour=c("#F8766D","#00BFC4",apha=0.7))+ # Add pairwise comparisons p-value 
...
ylab("Relative abundance (ppm)")+xlab("Seasonal variation")+ #设置x轴和y轴的标题
