...
library(ggpubr)
abundance <- read.csv("C:/Users/Robin.DESKTOP-0U5O684/Desktop/data/richness.csv", header=T)
compaired <- list(c("Dry season", "Wet season"))
windowsFonts(TNM = windowsFont("Times New Roman"))
ggplot(abundance,aes(x=Group,y=Richness,fill=Group))+ #”fill=“设置填充颜色
  geom_boxplot(size=0.6,fill="white",width=0.4, outlier.fill="white",outlier.color="white")+ #size设置箱线图的边框线和胡须的线宽度，fill设置填充颜色，outlier.fill和outlier.color设置异常点的属性
  geom_jitter(aes(fill=Group),width =0.2,shape = 21,size=8)+ #设置为向水平方向抖动的散点图，width指定了向水平方向抖动，不改变纵轴的值
  ...
  ylab("Richness(type)")+xlab("Seasonal variation")+ #设置x轴和y轴的标题
  ylim(295,460)+
