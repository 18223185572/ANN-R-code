...
p<-ggplot(data=saliarg, aes(x =ARGs, y =Abundance, size=Abundance,colour = Class))+# 拟合回归线段以及置信域(默认0.95/通过level参数可自定义))+  
  ...
  theme(legend.position= "bottom",
        ...
        axis.text.x=element_text(colour="black",face="plain",size=13), #设置x轴刻度标签的字体属性
        axis.text.y=element_text(size=13,face="plain"), #设置x轴刻度标签的字体属性
        axis.title.y=element_text(size = 13,face="plain"), #设置y轴的标题的字体属性
        axis.title.x=element_text(size = 13,face="plain"), #设置x轴的标题的字体属性
        plot.title = element_text(size=13,face="plain",hjust = 0.5))+ #设置总标题的字体属性
