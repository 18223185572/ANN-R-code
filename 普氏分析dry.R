#加载数据
micro <- read.csv("C:/Users/Robin.DESKTOP-0U5O684/Desktop/data/commundry.csv", header=T, row.names=1)
...
food_micro <- ggplot(plot) +
  geom_point(size = 5, alpha=0.90, aes(x = Axis.1, y = Axis.2, color = type)) +
  scale_color_manual(values = c("#F8766D", "#00BFC4")) +
  geom_line(aes(x= Axis.1, y=Axis.2, group=UserName), col = "darkgrey", alpha = 0.90,size=0.8) +
  ...
  annotate("text", x = 0.75e+05, y = 2e+05, label = paste0("Dry season, p<",pval), size = 9) +
