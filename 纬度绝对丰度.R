...
library(ggplot2)
library(ggExtra)
abso <- read.csv("C:/Users/Robin.DESKTOP-0U5O684/Desktop/data/absolute.csv", header=T)
windowsFonts(TNM = windowsFont("Times New Roman"))
p<-ggplot(data=abso, aes(x =Latitude, y =Absolute, size=Absolute, colour = Season))+# 拟合回归线段以及置信域(默认0.95/通过level参数可自定义))+  
  geom_point(alpha = .5) + # 使散点的透明度为0.5
  stat_smooth(method="lm")+
  stat_cor(data=abso, method = "spearman", size=8)+
  ...
  ylab("Absolute abundance(copies/g)")+xlab("Latitude") #设置x轴和y轴的标题
